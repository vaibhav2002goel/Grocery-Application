...
<body>
	...
	<div class="bg">
		...
		<div style="background-color:#CAEDFF; padding:2px;">
			<br>
			
			
			<div style="text-align:center;">
				<p style="font-size:16px font-family: Arial, Helvetica, sans-serif;">We specialize in Fruits,Vegetables And various types of Food Oils</p>
			</div>
			
			
			<div style="text-align:center;">
			
				<form action="BuyerSearchResult.jsp?search=<%= request.getParameter("searchValue") %>" method="get">
	    ...
	    <%!
			...
			String query1 = "select * from REPOSITORY where ProductCategory=?";
	    %>
		...
		%>
				...
				<div style="margin:0 1vw;" class="card w-25 text-bg-light mb-3 border-secondary " style="width: 18rem;">
				  ...
				  <div class="card-body">
				    <a href="AddToCart?username=<%= (String) session.getAttribute("username")%>&imagePath=<%= rs1.getString("ProductImage") %>&name="<%= rs1.getString("ProductName") %>"&description=<%= rs1.getString("ProductDescription") %>&price=<%= rs1.getString("ProductPrice") %>&category=<%= rs1.getString("ProductCategory")%>&quantity=<%= rs1.getString("ProductQuantity") %>" class="card-link">Add To Cart</a>
				  </div>	  
	       		</div>
		...
		%>
	       	...
	       	<div class="seven">
			<h1 style="font-weight:1000;font-size:50px;padding:20px;color:lightBlue;text-decoration:underline;" class="h1_1" id = "vegetables" >VEGETABLES</h1><br><br>
			</div>
			<div id="items">
		<%
			while(rs2.next()){	
		%>
				...
				<div style="margin:0 1vw;" class="card w-25 text-bg-light mb-3 border-secondary " style="width: 18rem;">
				  ...
				  <div class="card-body">
				    <a href="AddToCart?username=<%= (String) session.getAttribute("username")%>&imagePath=<%= rs2.getString("ProductImage") %>&name=<%= rs2.getString("ProductName") %>&description=<%= rs2.getString("ProductDescription") %>&price=<%= rs2.getString("ProductPrice") %>&category=<%= rs2.getString("ProductCategory")%>&quantity=<%= rs2.getString("ProductQuantity") %>" class="card-link">Add To Cart</a>
				  </div>	  
	       		</div>
		...
		<%
			...
			st3.setString(1,"Oils");
			
			ResultSet rs3 = st3.executeQuery();
		%>
	       	</div>
	       	<div class="seven">
			<h1  class="h1_1" id="oils" style="font-weight:1000;font-size:50px;padding:20px;color:lightBlue;text-decoration:underline ;">OILS</h1><br><br>
			</div>
			<div id="items">
		<%
			while(rs3.next()){
		%>
				<div style="margin:0 1vw;" class="card w-25 text-bg-light mb-3 border-secondary " style="width: 18rem;">
				  ...
				  <div class="card-body">
				    <a href="AddToCart?username=<%= (String) session.getAttribute("username")%>&imagePath=<%= rs3.getString("ProductImage") %>&name=<%= rs3.getString("ProductName") %>&description=<%= rs3.getString("ProductDescription") %>&price=<%= rs3.getString("ProductPrice") %>&category=<%= rs3.getString("ProductCategory")%>&quantity=<%= rs3.getString("ProductQuantity") %>" class="card-link">Add To Cart</a>
				  </div>	  
	       		</div>
		...
		%>
	       	...
	       	<div class="seven">
			<h1 class="h1_1" id="dairy" style="font-weight:1000;font-size:50px;padding:20px;color:lightBlue;text-decoration:underline ;">DAIRY</h1><br><br>
			</div>
			<div id="items">
		<%
			while(rs4.next()){
		...
		%>
				...
				<div style="margin:0 1vw;" class="card w-25 text-bg-light mb-3 border-secondary " style="width: 18rem;">
				  ...
				  <div class="card-body">
				    <a href="AddToCart?username=<%= (String) session.getAttribute("username")%>&imagePath=<%= rs4.getString("ProductImage") %>&name=<%= rs4.getString("ProductName") %>&description=<%= rs4.getString("ProductDescription") %>&price=<%= rs4.getString("ProductPrice") %>&category=<%= rs4.getString("ProductCategory")%>&quantity=<%= rs4.getString("ProductQuantity") %>" class="card-link">Add To Cart</a>
				  </div>	  
	       		</div>
		...
		%>
			...
			<div class="seven">
			<h1 class="h1_1" id="others" style="font-weight:1000;font-size:50px;padding:20px;color:lightBlue;text-decoration:underline ;">OTHERS</h1><br><br>
			</div>
			<div id="items">
		<%
			while(rs5.next()){
		...
		%>
				...
				<div style="margin:0 1vw;" class="card w-25 text-bg-light mb-3 border-secondary " style="width: 18rem;">
				  ...
				  <div class="card-body">
				    <a href="AddToCart?username=<%= (String) session.getAttribute("username")%>&imagePath=<%= rs5.getString("ProductImage") %>&name=<%= rs5.getString("ProductName") %>&description=<%= rs5.getString("ProductDescription") %>&price=<%= rs5.getString("ProductPrice") %>&category=<%= rs5.getString("ProductCategory")%>&quantity=<%= rs5.getString("ProductQuantity") %>" class="card-link">Add To Cart</a>
				  </div>	  
	       		</div>
